...
class CommandDispatcher {
   ...
   * @param {string} method - Verbo HTTP (POST por defecto, también admite GET)
   */
  async submit(url, data = {}, method = 'POST') {
    ...
    try {
      ...
      if (status < 200 || status > 299) {
        this._notifyError(status, text, requestUrl, bodyData);
        return;
      }
      ...
      this._notifyError(0, error.message, requestUrl, bodyData);
    }
  }
